...
{$ELSE}
  EhLibVCL,
{$ENDIF}
  Graphics, GridsEh, DBCtrls, DBCtrlsEh,
...
begin
  Result := False;
  Point1 := ClientToScreen(Point(Width, Height));
  NextParent := GetForm;
  ...
  begin
    ...
    begin
      ...
      NextParent.SetBounds(
        NextParent.Left,
        NextParent.Top,
        NextParent.Width + (NewFormSize.X - FLastFormSize.X),
...
//  Temp := Count div 2;
  ...
  begin
    ...
    begin
      ...
      NavButtons[I].SetBounds(X, 0, ButtonWidth + Space, H);
      Inc(X, ButtonWidth + Space);
    end
    else
      NavButtons[I].SetBounds (Width + 1, 0, ButtonWidth, Height);
  end;
...
begin
  ...
  NavButtons[nbDeleteEh].Enabled := Enabled and FDataLink.Active and
...
//    PaintControl.Canvas.Font := Grid.Font;
    ...
    begin
      ...
      WriteTextEh(PaintControl.Canvas, R, False, 0, 0, Text,
        taCenter, tlCenter, False, False, 0, 0, False);
...
begin
  ...
  begin
    ARect := GetClientRect;
    FillGradientEh(Canvas, ARect, Grid.Color, Grid.FixedColor)
  end else
    inherited Paint;
end;
...
{  if Focused then
  ...
  AGrid.SearchPanel.SearchingText := Text;
  if not FInternalChanging and (Text <> '') then
...
begin
  SpecInternalSetText('');
  TCustomDBGridEhCrack(Grid).SearchPanel.SearchingText := Text;
  TCustomDBGridEhCrack(Grid).SearchEditorMode := False;
...
//  if Grid.Flat then
  begin
    ARect := GetClientRect;
    Dec(ARect.Bottom);
    FromColor := Grid.Color;
    ToColor := Grid.FixedColor;
...
{$ENDIF}
//      FromColor := ApproximateColor(FromColor, HighlightColor, 30);
      ToColor := ApproximateColor(ToColor, HighlightColor, 100);
    end;
