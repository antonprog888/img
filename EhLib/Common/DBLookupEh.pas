...
{$ELSE}
  EhLibVCL,
{$ENDIF}
  DBLookupGridsEh, DBGridEh, DbUtilsEh;
...
begin
  ...
  begin
// This code bring to dead-loop when two Comboboxes are bound to the same ListSource
//    ResetDisplayText;
  end;
end;
...
procedure TCustomDBLookupComboboxEh.RefilterDropDownBoxListSource(FilterText: String);
var
...
begin
  ...
  if Assigned(FOnDropDownBoxApplyTextFilter) then
    FOnDropDownBoxApplyTextFilter(Self, ListDataSet, ListField, DropDownBox.ListSourceAutoFilterType, FilterText)
  else
    DefaultDropDownBoxApplyTextFilter(ListDataSet, ListField, DropDownBox.ListSourceAutoFilterType, FilterText);
