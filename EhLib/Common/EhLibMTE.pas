...
{                    (Build 6.0.01)                     }
{                                                       }
{    Copyright (c) 2003-2012 by Dmitry V. Bolshakov     }
{                                                       }
{*******************************************************}
...
type
  ...
  function WrapFieldName(FieldName: String): String;
  ...
  if Sender is TCustomDBGridEh then
    if TCustomDBGridEh(Sender).SortLocal then
      with TCustomDBGridEh(Sender) do
        begin
          OrderByStr := '';
          for i := 0 to DataGrouping.ActiveGroupLevelsCount - 1 do
          begin
            OrderByStr := OrderByStr + WrapFieldName(TColumnEh(DataGrouping.ActiveGroupLevels[i].Column).FieldName) + ' ';
            if DataGrouping.ActiveGroupLevels[i].SortOrder = soDescEh then
              OrderByStr := OrderByStr + ' DESC';
            OrderByStr := OrderByStr + ',';
          end;
          for i := 0 to SortMarkedColumns.Count - 1 do
          begin
            OrderByStr := OrderByStr + WrapFieldName(SortMarkedColumns[i].FieldName) + ' ';
            SortOrder := Center.GetSortOrderForSortMarker(TCustomDBGridEh(Sender),
              SortMarkedColumns[i], SortMarkedColumns[i].Title.SortMarker);
