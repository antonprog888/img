...
//    property DataType: TFieldType read FDataType write SetDataType default ftUnknown;
    if (DataField is TMTNumericDataFieldEh) and (FieldDefs[i].Precision <> 0) then
...
type
  TWriterCracker = class(TWriter) end;
...
begin
  if SortOrder <> '' then
  begin
    if (ParentNode.NodesCount = 0) or (FRecordsViewEh.CompareRecords(ParentNode[0].Rec, Rec)>0) then
    begin
      Result := 0;
      Exit;
    end;
    if (ParentNode.NodesCount > 1) and (FRecordsViewEh.CompareRecords(ParentNode[ParentNode.NodesCount-1].Rec, Rec)<0) then
    begin
