...
{$IFDEF EH_LIB_6}Variants, SqlTimSt, {$ENDIF}
{$IFDEF CIL}
  EhLibVCLNET,
...
{$ELSE}
  EhLibVCL,
{$ENDIF}
  DBGridEh, Db, Clipbrd, ComObj, StdCtrls;
...
//    FCol, FRow: Word;
  ...
  public
//    procedure ExportToStream(AStream: TStream; IsExportAll: Boolean; Options: TDBGridEhExportAsOLEXLSOptions); reintroduce; virtual;
    procedure ExportToStream(AStream: TStream; IsExportAll: Boolean); override;
...
begin
  CheckFirstCell;
  s := AnsiString(AnsiQuotedStr(FColCellParamsEh.Text, DBGridEhImpExpCsvQuoteChar));
  StreamWriteAnsiString(Stream, s);
//  Stream.Write(PChar(s)^, Length(s));
end;
...
begin
  CheckFirstCell;
  s := AnsiString(AnsiQuotedStr(Text, DBGridEhImpExpCsvQuoteChar));
  StreamWriteAnsiString(Stream, s);
//  Stream.Write(PChar(s)^, Length(s));
end;
...
begin
  ...
  for i := 0 to ColumnsList.Count - 1 do
  begin
    s := AnsiString(AnsiQuotedStr(ColumnsList[i].Title.Caption, DBGridEhImpExpCsvQuoteChar));
    if i <> ColumnsList.Count - 1 then
      s := s + Separator;
    StreamWriteAnsiString(Stream, s);
//    Stream.Write(PChar(s)^, Length(s));
...
{procedure TDBGridEhExportAsOLEXLS.ExportToStream(AStream: TStream;
  IsExportAll: Boolean; Options: TDBGridEhExportAsOLEXLSOptions);
begin
  FOptions := Options;
...
//  AType, AType1: TVarType;
//  TimeStamp, TimeStamp1: TSQLTimeStamp;
begin
...
begin
  if FDataRowCountMode then Exit;
//  FActiveSheet.ExcelApp.Visible := True;
...
{    S := FActiveSheet.Range['A1', 'A1'].NumberFormat;
    ...
    Range := FActiveSheet.Range[FActiveSheet.Cells.Item[FTitleRows+1, i+1],
    ...
    then
       Range.NumberFormat := AnsiString('@');
    if ExpCols[i].Checkboxes then
      Range.HorizontalAlignment := XLSAlignment[taCenter]
    else
      Range.HorizontalAlignment := XLSAlignment[ExpCols[i].Alignment];
    SetFontIfNeed(Range, ExpCols[i].Font);
  end;

  TopLeft := 'A'+IntToStr(FTitleRows+1);
  BottomRight := FActiveSheet.Cells.Item[FTitleRows+FDataRowCount, FCurCol-1];
  FActiveSheet.Range[TopLeft, BottomRight].Value := FVarValues;
  Range := FActiveSheet.Range[TopLeft, BottomRight];
  Range.Value := FVarValues;
  SetFont(Range, DBGridEh.Font);
...
begin
 ...
 // Disabling Excel reaction to fastering out data.
...
//  ExcelApp := Null;
  ...
  FDefaultSizeDelta := FActiveSheet.Range['A1', 'A1'].Font.Size / DBGridEh.Font.Size;

{  DBGridEh.DataSource.DataSet.DisableControls;
  DBGridEh.SaveBookmark;
  DBGridEh.DataSource.DataSet.First;
  try
    while DBGridEh.DataSource.DataSet.Eof = False do
    begin
      Inc(FDataRowCount);
...
(*      for i := 0 to ColumnsList.Count - 1 do
      begin
        ...
        begin
          ...
          FActiveSheet.Range[FActiveSheet.Cells[FCurRow, 1+i], FActiveSheet.Cells[FCurRow+RowSpan-1, 1+i+ColSpan-1]].Select;
          FExcelApp.Selection.MergeCells:=True;
          FActiveSheet.Cells(FCurRow, 1+i) := THeadTreeNode(TList(ListOfHeadTreeNodeList.Items[0]).Items[i]).Text;
          FActiveSheet.Cells[FCurRow, 1+i].HorizontalAlignment := XLSAlignment[ColumnsList[i].Title.Alignment];
          FActiveSheet.Cells[FCurRow, 1+i].WrapText := True;
          SetFontIfNeed(FActiveSheet.Cells[FCurRow, 1+i], ColumnsList[i].Title.Font);
          if ColumnsList[i].Title.Orientation = tohVertical then
...
begin
  ...
  FActiveSheet.Cells[FCurRow, FCurCol].HorizontalAlignment := XLSAlignment[Footer.Alignment];
...
begin
  if (DBGridEh.Font.Style <> Font.Style) or
     ...
     (DBGridEh.Font.Size <> Font.Size) then
    SetFont(Range, Font);
end;
...
begin
  ...
  Range.Font.Size := Integer(Round(Font.Size * FDefaultSizeDelta));
end;
