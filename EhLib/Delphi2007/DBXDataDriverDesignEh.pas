...
{                     EhLib v6.0                        }
{                                                       }
{         TDBXDesignDataBaseEh (Build 6.0.01)           }
{                                                       }
...
(*
// All msut work in Object Inspector

{ TDBXConnectionProviderEhEditor }

type

  TDBXConnectionProviderEhEditor = class(TComponentEditor)
    procedure ExecuteVerb(Index: Integer); override;
    function GetVerb(Index: Integer): string; override;
    function GetVerbCount: Integer; override;
  end;

procedure TDBXConnectionProviderEhEditor.ExecuteVerb(Index: Integer);
begin
  case Index of
    0: if EditDatabase(TDBXConnectionProviderEh(Component).DesignConnection.DataBase) then
         Designer.Modified;
  end;
end;

function TDBXConnectionProviderEhEditor.GetVerb(Index: Integer): string;
begin
  case Index of
...
function TDBXConnectionProviderEhEditor.GetVerbCount: Integer;
begin
  Result := 0;
  if (Component <> nil) and (Component is TDBXConnectionProviderEh) then
    if TDBXConnectionProviderEh(Component).DesignConnection.UseDesignConnection then
      Inc(Result);
end;
*)
...
{$IFDEF DESIGNTIME}
  ...
  RegisterComponents('EhLib', [TDBXConnectionProviderEh]);
