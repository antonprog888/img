...
{$ENDIF}
  ...
  DBGridEh, {DBGrids,} DBLookupEh, DBGridEhGrouping,
{$IFDEF CIL}
  EhLibVCLNET,
{$ELSE}
...
{ TDBGridEhDesignControler }
  ...
  public
    ...
    function IsObjectBelongToGrid(Grid: TCustomDBGridEh; AObject: TPersistent): Boolean;
...
//        ShowMessage('GlobalHideConnectedComponens:' + BoolToStr(GlobalHideConnectedComponens));
    ...
    1: Result := '-';
    2: Result := EhLibVerInfo + ' ' + EhLibEditionInfo;
  end;
end;
...
{$IFDEF CIL}
{$ELSE}
      and TCustomDBGridEhCracker(DBGridEh).OutBoundaryData.GetOutBoundaryRect(OutBRect, cbtTopEh)
{$ENDIF}
...
{    if FOldSelection <> nil then
      for j := 0 to FOldSelection.Count-1 do
        if IsObjectBelongToGrid(TCustomDBGridEh(FGridList[i]), FOldSelection.Items[j]) then
          TCustomDBGridEh(FGridList[i]).Invalidate;}

    if ASelection <> nil then
      for j := 0 to ASelection.Count-1 do
{$IFDEF EH_LIB_6}
        if IsObjectBelongToGrid(TCustomDBGridEh(FGridList[i]), ASelection.Items[j]) then
{$ELSE}
        if IsObjectBelongToGrid(TCustomDBGridEh(FGridList[i]), ExtractPersistent(ASelection.Items[j])) then
{$ENDIF}
          TCustomDBGridEh(FGridList[i]).Invalidate;
...
function TDBGridEhDesignControler.IsObjectBelongToGrid(
  Grid: TCustomDBGridEh; AObject: TPersistent): Boolean;
var
  AOwner: TPersistent;
begin
  Result := False;
...
{$ELSE}
  if Supports(FindRootDesigner(Component), IFormDesigner, LDesigner) then
{$ENDIF}
  begin
