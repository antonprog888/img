...
{$ENDIF}
  ...
  DBGridEhGrouping, DBGridEhToolCtrls, ToolCtrlsEh;
...
begin
//  if DDB <> DataDriver.DesignDataBase then
//  begin
    DataDriver.DesignDataBase := DDB;
    if DataDriver.DesignDataBase <> nil then
    begin
//      DesignDriver := TDesignDataBaseEh(DataDriver.DesignDataBase).CreateDesignCopy(DataDriver);
...
begin
  ...
  begin
//    ShowMessage('DesignerNotify = nil');
    Exit;
  end;
{$IFDEF DESIGNTIME}
  Result := GetProjectFromDesigner(DesignerNotify as IDesigner);
{$ENDIF}
//  (DesignerNotify as IDesigner).ModuleFileNames(N1,N2,N3);
//  Result := ExtractFilePath(Prj.FileName);
//  Result := ExtractFilePath(N1);
  Result := ExtractFilePath(Result);
  Result := ExcludeTrailingPathDelimiter(Result);
//  Result := N1 + '-' + N2 + '-' + N3;
//  Result := (DesignerNotify as IDesigner).GetPrivateDirectory;
//  Result := (DesignerNotify as IDesigner).GetPathAndBaseExeName;
end;
...
begin
  ...
  begin
//    ShowMessage('Ok');
//    p := (ms.Modules[pi] as IOTAProject);
    if Supports(ms.Modules[pi], IOTAProject, p) then
    begin
