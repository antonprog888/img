...
function DefaultLocateDatasetTextEh(AGrid: TCustomDBGridEh;
  const FieldName, Text: String; AOptions: TLocateTextOptionsEh;
  Direction: TLocateTextDirectionEh; Matching: TLocateTextMatchingEh): Boolean;
...
uses
  DBConsts, DBGridEhFindDlgs, Contnrs, EhLibConsts;
...
begin
  ...
  begin
    ...
    begin
      ...
      if DefaultOperator in [fdoBeginsWith, fdoBeginsWith, fdoContains, fdoDoesntContain] then
      begin
        v := VarToStr(v);
  ...
  //        raise Exception.Create(SUnexpectedExpressionBeforeNullEh + Exp)
      ...
      else if not (op1 in [foValue, foIn]) then
        raise Exception.Create(SUnexpectedExpressionAfterOperatorEh + Exp);
    end else
      NoClosedComma := True;
    FExpression.Operand1 := v;
    ...
    begin
      ...
      try
        ...
        begin
          if i > 0 then
            Result := Result + ', ';
          Result := IntToStr(TField(FieldList[i]).FieldNo);
        end;
      finally
...
var
  ...
  begin
    ...
    begin
      if AGrid.Selection.SelectionType = gstRectangle then
        if AGrid.VisibleColumns[FCurInListColIndex] <> AGrid.Columns[AGrid.Selection.Rect.RightCol] then
        begin
        ...
        if FCurInListColIndex < AGrid.Selection.Columns.Count-1 then
        begin
          Inc(FCurInListColIndex);
          Result := True;
        end else
          Result := False;
    end else if FCurInListColIndex < AGrid.VisibleColumns.Count-1 then
    begin
      Inc(FCurInListColIndex);
      Result := True;
    end else
  ...
  begin
    ...
    begin
      if AGrid.Selection.SelectionType = gstRectangle then
        if AGrid.VisibleColumns[FCurInListColIndex] <> AGrid.Columns[AGrid.Selection.Rect.LeftCol] then
        begin
        ...
        begin
          ...
          Result := False;
    end else if FCurInListColIndex > 0 then
    begin
      Dec(FCurInListColIndex);
      Result := True;
    end else
    ...
    if ToFirstCol then
      if ltoInsideSelection in AOptions then
        case AGrid.Selection.SelectionType of
          ...
          gstRectangle: FCurInListColIndex := AGrid.VisibleColumns.IndexOf(AGrid.Columns[AGrid.Selection.Rect.LeftCol]);
      ...
      if ltoInsideSelection in AOptions then
        case AGrid.Selection.SelectionType of
          gstRecordBookmarks: FCurInListColIndex := AGrid.VisibleColumns.Count-1;
          gstRectangle: FCurInListColIndex := AGrid.VisibleColumns.IndexOf(AGrid.Columns[AGrid.Selection.Rect.RightCol]);
          gstColumns: FCurInListColIndex := AGrid.Selection.Columns.Count-1;
          gstAll: FCurInListColIndex := AGrid.VisibleColumns.Count-1;
          gstNon: FCurInListColIndex := AGrid.VisibleColumns.Count-1;
        end
      else
...
//    FCurInListColIndex := AGrid.SelectedIndex;
    ...
    begin
      if AGrid.Selection.SelectionType = gstRectangle then
        if (AGrid.SelectedIndex < AGrid.Selection.Rect.LeftCol) or
           (AGrid.SelectedIndex > AGrid.Selection.Rect.RightCol) then
          FCurInListColIndex := AGrid.VisibleColumns.IndexOf(AGrid.Columns[AGrid.Selection.Rect.LeftCol])
        else
          FCurInListColIndex := AGrid.VisibleColumns.IndexOf(AGrid.Columns[AGrid.SelectedIndex])
      else // AGrid.Selection.SelectionType = gstColumns
      begin
        FCurInListColIndex := AGrid.Selection.Columns.IndexOf(AGrid.Columns[AGrid.SelectedIndex]);
        if FCurInListColIndex < 0 then
          FCurInListColIndex := 0;
      end;
    end else
...
//        if ltoIgnoteCurrentPosEh in AOptions then
        ...
        begin
          if CheckEofBof then
            ...
            if (ltoInsideSelection in AOptions) and
              (AGrid.Selection.SelectionType = gstColumns)
            then DataText := ColText(AGrid.Selection.Columns[FCurInListColIndex])
            else DataText := ColText(AGrid.VisibleColumns[FCurInListColIndex]);
...
//              IsFirstTry := False;
          ...
          begin
            ...
            if (ltoInsideSelection in AOptions) and
              (AGrid.Selection.SelectionType = gstColumns)
            then AGrid.SelectedIndex := AGrid.Selection.Columns[FCurInListColIndex].Index
            else AGrid.SelectedIndex := AGrid.VisibleColumns[FCurInListColIndex].Index;
//            IsFirstTry := False;
            Break;
          end;
  ...
  if DataSet <> nil then
    ...
    begin
      ...
      begin
        ...
        begin
          FilterStr := FilterStr + '[' + TField(FieldList[i]).FieldName + '] like ''' + swc1 +
...
begin
  Result := Field.DataType in [ftString, ftMemo, ftFmtMemo, ftMemo, ftFmtMemo,
