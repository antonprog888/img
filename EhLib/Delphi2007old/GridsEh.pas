...
//    function GetTopNonExtendVertLineColor  Fill in area not occupied by cells
//    function GetTopNotOccupiedAreaVertLineColor(ACol: Integer): TColor; virtual;
//    function GetBottom TopNotOccupiedAreaVertLineColor(ACol: Integer): TColor; virtual;
...
//    property ExcludeTitle: Boolean read FExcludeTitle write SetExcludeTitle default True;
//    property ExcludeFooter: Boolean read FExcludeFooter write SetExcludeFooter default True;
...
//{$ENDIF}
  protected
    ...
    function CheckCellLine(ACol, ARow: Integer; IsVert: Boolean): Boolean;
...
{$ENDIF}
    ...
    procedure StartColMoving(ColIndex: Integer; X, Y: Integer; var DrawInfo: TGridDrawInfoEh);
...
begin
  ...
  if CheckCellLine(ACol, ARow, True) then
  begin
    if not (goVertLine in Options) then
      Dec(Result.Right, GridLineWidth)
  end else
  begin
    if goVertLine in Options then
      Inc(Result.Right, GridLineWidth);
  end;

  if CheckCellLine(ACol, ARow, False) then
  begin
...
{function ArrayOfInteger(SetArray: array of Integer): TIntegerDynArray;
begin
  Result := TIntegerDynArray(Pointer(SetArray));
end;}
...
//  Exit; //tmp
  ...
  for ICol := Col to FullColCount-1 do
  begin
...
//      Inc(TopStart, RowHeights[IRow]);
      ...
      if RIsDraw and (ColWidths[ICol] >= 0) then
      begin
        if RIsExtent then ExtentBit := 1 else ExtentBit := 0;
        if not (goVertLine in Options)
          then LinePosFit := -GridLineWidth
          else LinePosFit := 0;
      ...
      if BIsDraw and (RowHeights[IRow] >= 0) then
      begin
        if not (goHorzLine in Options) then
        begin
    ...
    Inc(LeftStart, ColWidths[ICol] + DrawInfo.Horz.EffectiveLineWidth);
  end;
...
begin
  ...
  begin
    ...
    if UseRightToLeftAlignment then
      RectRgn := CreateRectRgn(FPaintDrawInfo.Horz.FullGridExtent - FPaintDrawInfo.Horz.ContraExtent,
        0, FPaintDrawInfo.Horz.FullGridExtent, FPaintDrawInfo.Vert.ContraExtent)
    else
...
{      Canvas.Brush.Color := FInternalColor;
      if not GridBackgroundFilled then
        Canvas.FillRect(Rect(Horz.GridBoundary, Vert.NullBorderIndent, Horz.GridExtent, Vert.GridBoundary));
}
    end;
...
{      Canvas.Brush.Color := FInternalColor;
      if not GridBackgroundFilled then
      begin
        Canvas.FillRect(Rect(Horz.NullBorderIndent, Vert.GridBoundary, Horz.FixedBoundary+1, Vert.GridExtent));
...
{    if goExtendVertLines in Options then
    begin
      DrawGradientUnderDataLines(FPaintDrawInfo, 0, Horz.FixedBoundary, clNone);
      DrawGradientUnderDataLines(FPaintDrawInfo, Horz.FixedBoundary+1, Horz.GridBoundary, clNone);
    end}
  end;
...
function TCustomGridEh.CheckCellLine(ACol, ARow: Longint; IsVert: Boolean): Boolean;
var
  BorderType: TGridCellBorderTypeEh;
  IsDraw: Boolean;
  BorderColor: TColor;
...
//            Include(DrawState, gdSelected);
  ...
  function GetCellBorderColor(ACol, ARow: Integer; BorderType: TGridCellBorderTypeEh): TColor;
  var
    IsDraw, IsExtent: Boolean;
  begin
    CheckDrawCellBorder(ACol, ARow, BorderType, IsDraw, Result, IsExtent);
  end;
...
//       GridLineColors.GetCellColor(0, RowCount){FFixedLineColor}, FixedColor, Canvas, DrawLinesInfo);
      DrawLinesEh(DrawInfo, goFixedHorzLine in Options, goFixedVertLine in Options,
        0, RowCount,
        [Horz.NullBorderIndent, Vert.ContraExtent+1, Horz.NoFrozenBoundary, Vert.FullGridExtent],
        GridLineColors.GetCellColor(0, RowCount){FFixedLineColor}, FixedColor, Canvas, DrawLinesInfo);
      ...
      if Horz.FrozenCelCount > 0 then
        DrawFirstContraLine(Horz.FixedBoundary-1, Vert.ContraExtent+1,
          ...
          GetCellBorderColor(FixedCols-1, RowCount, cbtRightEh)
      ...
      DrawFirstContraLine(Horz.NullBorderIndent, Vert.ContraExtent,
        ...
        GetCellBorderColor(0, RowCount, cbtTopEh)
...
//        Horz.FixedBoundary, Vert.ContraExtent+1, Horz.GridBoundary, Vert.FullGridExtent);
    if (FDataOffset.cx <> 0) or (FDataOffset.cy <> 0) then
      ...
      DrawFirstContraLine(Horz.OutViewVisCellStart, Vert.ContraExtent,
        ...
        GetCellBorderColor(LeftCol, RowCount, cbtTopEh)
...
//      RestoreDataDrawOriginClipRegion(RCR);
      ...
      DrawLinesEh(DrawInfo, goFixedHorzLine in Options, goFixedVertLine in Options,
        ColCount, 0,
        [Horz.ContraExtent+1, Vert.NullBorderIndent, Horz.FullGridExtent, Vert.NoFrozenBoundary],
      ...
      if Vert.FrozenCelCount > 0 then
        DrawFirstContraLine(Horz.ContraExtent+1, Vert.FixedBoundary-1,
          ...
          GetCellBorderColor(ColCount, FixedRows-1, cbtLeftEh)
      ...
      DrawFirstContraLine(Horz.ContraExtent, Vert.NullBorderIndent,
        ...
        GetCellBorderColor(ColCount, 0, cbtLeftEh)
        {GridLineColors.GetLeftBorderCellColor(ColCount, FixedRows-FrozenRowCount)}{FFixedLineColor});

      DrawLinesEh(DrawInfo, goHorzLine in Options, goVertLine in Options,
        ColCount, TopRow,
        [Horz.ContraExtent+1, Vert.OutViewVisCellStart, Horz.FullGridExtent, Vert.GridBoundary],
        GridLineColors.GetCellColor(ColCount, TopRow){LineColor}, Color, Canvas, DrawLinesInfo);
      DrawFirstContraLine(Horz.ContraExtent, Vert.OutViewVisCellStart,
        Horz.ContraExtent, Vert.GridBoundary,
        GetCellBorderColor(ColCount, TopRow, cbtLeftEh){FFixedLineColor});
...
// Vert
    ...
    DrawContraCells(ColCount, TopRow,
      Horz.ContraExtent+1, Vert.OutViewVisCellStart, Horz.FullGridExtent, Vert.GridBoundary, //!! clip
      FInternalColor, []);
  ...
  begin
    ...
    begin
      ...
      if ContraCelCount > 0 then
        Dec(ContraExtent, GridLineWidth); //Bottom line
...
begin
  ...
  //Dec(Result.Right, FImageHorzMargin);
  Inc(Result.Top, FImageVertMargin);
  //Dec(Result.Bottom, FImageVertMargin);
