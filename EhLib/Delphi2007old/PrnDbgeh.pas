...
{$ELSE}
  EhLibVCL,
{$ENDIF}
  ImgList, PrViewEh, Forms, DB, GridsEh, DBGridEhGrouping, StdCtrls, ComCtrls;
...
const Macros: array[0..5] of String = ('&[Page]', '&[ShortDate]', '&[Date]',
  ...
  begin
    if DBGridEh.DataGrouping.IsGroupingWorks and
      (DBGridEh.DataGrouping.GroupDataTree.FlatVisibleCount > 0)
    then
  ...
  begin
    if DBGridEh.DataGrouping.IsGroupingWorks and
        (DBGridEh.DataGrouping.GroupDataTree.FlatVisibleCount > 0) then
    begin
      APBm.InDS := True;
...
//        SetPen;
  ...
  begin
    Result := 0;
    if DBGridEh.DataGrouping.IsGroupingWorks and
       (DBGridEh.DataGrouping.GroupDataTree.FlatVisibleItem[APBmFrom.RecNo].NodeType = dntDataGroupEh) then
    begin
      ...
      Result := WriteTextEh(VPrinter_Canvas, DrawRect, False, 0, 0, FColCellParamsEh.Text,
    ...
    begin
      ...
      begin
        ...
        if APBmFrom.InDS and
           DBGridEh.DataGrouping.IsGroupingWorks and
           (DBGridEh.DataGrouping.GroupDataTree.FlatVisibleItem[APBmFrom.RecNo].NodeType = dntDataGroupEh)
        then
...
//      PrintPageColontitle(FooterRect,False,False);
      ...
      begin
        ...
        if CurPBm.InDS and
           DBGridEh.DataGrouping.IsGroupingWorks and
           (DBGridEh.DataGrouping.GroupDataTree.FlatVisibleItem[CurPBm.RecNo].NodeType = dntDataGroupEh) then
        begin
          for i := FromCol to ToCol do
          ...
          begin
            ...
            if CurPBm.InDS and DBGridEh.DataGrouping.IsGroupingWorks and (i = 0) then
            begin
...
//    PolyLengths := TList.Create;
   ...
   //FromRow := 0; PageNo := 1;
    ...
    begin
      ...
      PrintOffset := 0;
      DBGridEh.DataSource.DataSet.First;
{ TODO -cCheck : Dynamic array automaticaly zeroize}
