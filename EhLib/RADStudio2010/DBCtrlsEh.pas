...
{$ENDIF}
  ...
  ToolCtrlsEh, ActnList, Consts, Math, DynVarsEh;
...
begin
  Result := (FText <> '') and (not FControl.Focused) and (FControl.Text = '');
end;
...
//  if not (NewStyleControls and FControl.Ctl3D) and (FControl.BorderStyle = bsSingle) then
...
//  SetDesigning(True);
  ...
  ControlStyle := ControlStyle + [csReplicatable, csCaptureMouse{, csOpaque}];
...
//  FButtonsPanel := TWinControl.Create(Self);
  ...
  FButtonsPanel.SetBounds(0,0,0,0);
end;
...
//  if inherited UseRightToLeftAlignment
//    then Indent := 0
//    else Indent := FButtonsPanel.ClientWidth;

  for i := 0 to Length(FEditButtonControlList)-1 do
  begin
    with FEditButtonControlList[i] do
    begin
      EditButtonControl.SetBounds(Indent, ButtonRect.Top, EditButtonControl.Width, MinButtonHeight);
  ...
  if Flat and (Indent > 0) and not ThemesEnabled then
    Dec(Indent);
...
{  if inherited UseRightToLeftAlignment
  ...
  if Indent > 0 then
  begin
    FButtonsPanel.SetBounds(ClientWidth-FButtonWidth, 0, FButtonWidth, MinButtonHeight);
...
begin
  if FInplaceMode
    then Result := HandleAllocated and IsWindowVisible(Handle)
...
//    if TPopupListboxEh(FPopupListbox).CanCloseMouseUp(Point(X, Y), Button, Shift) then
