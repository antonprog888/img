...
var
  ...
  v: Variant;
const
  GrOpOfAggFuncs: array [TGroupOperation] of TAggrFunctionEh = (agfSumEh, agfAvg, agfCountEh);
begin
...
begin
  if not (csLoading in FOwner.ComponentState) and (Active = True) and
          Assigned(DataSet) and (DataSet.Active = True) and
...
//                if (Item.FieldName = '') or not FDataSet.FieldByName(Item.FieldName).IsNull then
              ...
              goAvg:
                begin
                  ...
                  Item.FSumValueAsSum := Item.FSumValueAsSum + FieldArr[i].AsFloat;
//                  if (FDataSet.FieldByName(Item.FieldName).IsNull = False) then
//                    Inc(Item.FNotNullRecordCount);
