...
uses SysUtils, Classes, Controls, DB, Windows, Messages, Forms,
...
{$ELSE}
  EhLibVCL,
{$ENDIF}
  ToolCtrlsEh, DynVarsEh, DBCommon, MemTableDataEh;
type
...
begin
  Result := False;
//  ShowMessage('TDataDriverEh.ConnectionIsPostponed=False');
end;
...
//  Field := ReaderDataSet.Fields[MemRec.DataStruct[DataFieldIndex].Index];  //Future optimization
...
//  else
//    raise Exception.Create('GetConnectionProvider=nil: '+ Name);
//    ShowMessage('GetConnectionProvider=nil: '+ Name);
end;
...
begin
  Result := False;
//  ShowMessage('TConnectionProviderEh.ConnectionIsPostponed=False');
end;

constructor TConnectionProviderEh.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
//  ShowMessage('TConnectionProviderEh Created');
end;
