...
{$IFDEF EH_LIB_17} System.Generics.Collections, Winapi.ActiveX, System.Types, {$ENDIF}
{$IFDEF CIL}
...
uses Forms, DbConsts, Math, EhLibConsts,
{$IFDEF EH_LIB_6}
  SqlTimSt, FmtBcd,
{$ENDIF}
  TypInfo;
...
begin
  ...
  else if UseMemRec and (RecView <> nil) and (Field.FieldKind in [fkCalculated, fkLookup]) then
...
function TRecBuf.GetOldValue(Field: TField): Variant;
begin
  if UseMemRec and (MemRec <> nil) and (Field.FieldNo > 0) then
  ...
  begin
    try
      ...
      begin
        ...
        BufferToRecBuf(TempBuffer).NeedUpdateCalcFields := True;
        try
          GetCalcFields(TempBuffer);
        finally
          BufferToRecBuf(TempBuffer).NeedUpdateCalcFields := False;
        end;
      end;
...
//    Exit;
    RecordToBuffer(Item, dvvValueEh, TempBuffer, -1);
    BufferToRecBuf(TempBuffer).RecView := RecView;
    BufferToRecBuf(TempBuffer).NeedUpdateCalcFields := True;
// TODO : Check the slowing down of IndexOf
    BufferToRecBuf(TempBuffer).RecordNumber := FRecordsView.IndexOf(Item);
    try
      GetCalcFields(TempBuffer);
    finally
      BufferToRecBuf(TempBuffer).NeedUpdateCalcFields := False;
...
//      FRecordsView.MemTableData.RecordsList.CachedUpdates := True;
      ...
      FRecordPos := FRecordsView.IndexOf(Rec);
      if FRecordPos >= 0 then
        SetRecViewLookupBuffer(ActiveBuffer, FRecordsView.RecordView[FRecordPos]);
    end else
...
begin
  ...
  begin
    ...
    if (DataDriver <> nil) and not (mtoPersistentStructEh in Options) then
    begin
...
//        Result := RecIndex;
      ...
      begin
        ...
        for I := 0 to FRecordsView.Count-1 do
        begin
...
begin
  ...
  begin
    ...
    begin
        for I := 0 to AOrderByList.Count - 1 do
        begin
          if AOrderByList[I].FieldIndex >= 0 then
          begin
