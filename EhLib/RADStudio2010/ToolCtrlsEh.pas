...
//    FControlHotImages: TCustomImageList;
    FHotIndex: Integer;
    FNormalImages: TCustomImageList;
    FPressedIndex: Integer;
    FDisabledImages: TCustomImageList;
//    FControlHotIndex: Integer;
//    procedure SetControlHotImages(const Value: TCustomImageList);
//    procedure SetControlHotIndex(const Value: Integer);
...
//    property ControlHotImages: TCustomImageList read FControlHotImages write SetControlHotImages;
...
//    property ControlHotIndex: Integer read FControlHotIndex write SetControlHotIndex default 0;
...
//    function IsMouseUpCloseListbox(MousePos: TPoint; Button: TMouseButton; Shift: TShiftState): Boolean; virtual;
...
{ TPopupListboxItemEh }
  ...
  TPopupListboxItemEh = class(TObject)
  private
  protected
    function CanFocus(Sender: TPopupListboxEh; ItemIndex: Integer): Boolean; virtual;
    procedure DrawItem(Sender: TPopupListboxEh; ItemIndex: Integer; ARect: TRect; State: TOwnerDrawState); virtual;
    procedure MouseDown(Sender: TPopupListboxEh; ItemIndex: Integer; InItemPos: TPoint; Button: TMouseButton; Shift: TShiftState); virtual;
    procedure MouseMove(Sender: TPopupListboxEh; ItemIndex: Integer; InItemPos: TPoint; Shift: TShiftState); virtual;
    procedure MouseUp(Sender: TPopupListboxEh; ItemIndex: Integer; InItemPos: TPoint; Button: TMouseButton; Shift: TShiftState; var IsCloseListbox: Boolean); virtual;
    procedure KeyPress(Sender: TPopupListboxEh; ItemIndex: Integer; var Key: Char; Shift: TShiftState; var IsCloseListbox: Boolean); virtual;
  public
    function CloseOnExecute(Sender: TPopupListboxEh; ItemIndex: Integer): Boolean; virtual;
//    function IsMouseUpCloseListbox(Sender: TPopupListboxEh; ItemIndex: Integer; InItemPos: TPoint; Button: TMouseButton; Shift: TShiftState): Boolean; virtual;
    procedure Execute(Sender: TPopupListboxEh; ItemIndex: Integer; InItemPos: TPoint; Shift: TShiftState); virtual;
  end;
...
function FieldValueToDispayValue(AValue: Variant; Field: TField; ADisplayFormat: String): String;
...
// Converted from std C++ func hilisut
...
var
  ...
  function geli(s: String; p0, W: Integer): Integer;
  var
    i, cw, ls0: Integer;
  begin
    cw := 0;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        da[Length(da)-1].X :=
          C.TextWidth(Copy(T, lsa[im].Start+1, Pos-lsa[im].Start));
        ...
        da[Length(da)-1].Y := StringSize.cy * im;
        da[Length(da)-1].StrStart := Pos;
        da[Length(da)-1].StrLength := Len;
        da[Length(da)-1].gtdu := gtdu;
      end;
    end;
  end;
...
begin
  ...
  begin
    Options := ETO_CLIPPED;
    ddr := Rect(da[i].X + AR.Left + X, da[i].Y + AR.Top + Y, 0, 0);
    ddr.Right := ddr.Left +
      C.TextWidth(Copy(T, da[i].StrStart+1, da[i].StrLength));
...
//  DefCheckBoxWidth, DefCheckBoxHeight: Integer;
begin
{  if Flat then
  begin
    DefCheckBoxWidth := DefaultFlatCheckBoxWidth;
    DefCheckBoxHeight := DefaultFlatCheckBoxHeight;
  end else
  begin
    DefCheckBoxWidth := DefaultCheckBoxWidth;
    DefCheckBoxHeight := DefaultCheckBoxHeight;
  end;}
...
//const
//  CheckBoxThemedButton: array[TCheckBoxState, Boolean, ] of Integer = (0, DFCS_PUSHED {? or DFCS_FLAT});
begin
  SaveRgn := 0;
  OldRgn := 0;
  DrawRect := R;
...
begin
  ...
  begin
    ADrawRect := Rect(0,0,ADrawImages.Width,ADrawImages.Height{-1});
    ADrawRect := CenteredRect(ARect, ADrawRect);
    ADrawRect.Bottom := ADrawRect.Bottom{ + 1};
    if FButtonNum > 0 then
...
{    if Supports(Owner, IComboEditEh, ComboEdit) and ComboEdit.IsHot then
      Result := ebstControlHotEh
    else}
      Result := ebstNormalEh
...
begin
  ...
  begin
    ...
    MasterAbsRect.TopLeft := Parent.ClientToScreen(Point(0, 0));
    MasterAbsRect.Bottom := MasterAbsRect.Top + Parent.Height;
    MasterAbsRect.Right := MasterAbsRect.Left + Parent.Width;
    MonInfo.cbSize := SizeOf(MonInfo);
{$IFDEF CIL}
    GetMonitorInfo(MonitorFromRect(MasterAbsRect, MONITOR_DEFAULTTONEAREST), MonInfo);
{$ELSE}
...
begin
  if UseItemObjects and (Index < Items.Count) and (Items.Objects[Index] is TPopupListboxItemEh) then
    TPopupListboxItemEh(Items.Objects[Index]).DrawItem(Self, Index, ARect, State)
  else
    DefaultDrawItem(Index, ARect, State);
end;

procedure TPopupListboxEh.DefaultDrawItem(Index: Integer; ARect: TRect;
...
begin
  ...
  if (ItemIndex >= 0) and UseItemObjects and (Items.Objects[ItemIndex] is TPopupListboxItemEh) then
    TPopupListboxItemEh(Items.Objects[ItemIndex]).
...
begin
  ...
  if (Index >= 0) and UseItemObjects and (Items.Objects[Index] is TPopupListboxItemEh) then
    TPopupListboxItemEh(Items.Objects[Index]).
      MouseMove(Self, Index, Point(X,Y), Shift);
  FMousePos := Point(X, Y);
end;
...
begin
  ...
  if (Index >= 0) and UseItemObjects and (Items.Objects[Index] is TPopupListboxItemEh) then
    TPopupListboxItemEh(Items.Objects[Index]).
      MouseDown(Self, Index, Point(X,Y), Button, Shift);
end;
...
{$ENDIF}
{$ENDIF}
    ItemIndex := Integer(itemID);
    if (ItemIndex < Items.Count) and UseItemObjects and (Items.Objects[ItemIndex] is TPopupListboxItemEh) then
      if not TPopupListboxItemEh(Items.Objects[ItemIndex]).CanFocus(Self, ItemIndex) then
      begin
...
{$IFDEF EH_LIB_5}
  ...
  {$ENDIF}
{$ELSE}
        TOwnerDrawState(WordRec(LongRec(itemState).Lo).Lo) := State;
{$ENDIF}
      end;
  end;
...
procedure TPopupListboxEh.InvalidateIndex(AItemIndex: Integer);
var
  AItemRect: TRect;
begin
...
{function TPopupListboxEh.IsMouseUpCloseListbox(MousePos: TPoint; Button: TMouseButton; Shift: TShiftState): Boolean;
var
  Index: Integer;
  AItemRect: TRect;
begin
  Result := True;
  Index := ItemAtPos(MousePos, True);
  if Index >= 0 then
  begin
    AItemRect := ItemRect(Index);
    if (ExtItems <> nil) and (ExtItems.Objects[Index] is TPopupListboxItemEh) then
      Result := TPopupListboxItemEh(ExtItems.Objects[Index]).IsMouseUpCloseListbox(Self, Index, MousePos, Button, Shift)
    else if Items.Objects[Index] is TPopupListboxItemEh then
      Result := TPopupListboxItemEh(Items.Objects[Index]).IsMouseUpCloseListbox(Self, Index, MousePos, Button, Shift)
    else
      Result := True;
  end;
end;}
...
procedure TPopupListboxItemEh.DrawItem(Sender: TPopupListboxEh;
  ItemIndex: Integer; ARect: TRect; State: TOwnerDrawState);
begin
  Sender.DefaultDrawItem(ItemIndex, ARect, State);
end;

procedure TPopupListboxItemEh.MouseDown(Sender: TPopupListboxEh;
  ItemIndex: Integer; InItemPos: TPoint; Button: TMouseButton;
  Shift: TShiftState);
begin
end;

procedure TPopupListboxItemEh.MouseMove(Sender: TPopupListboxEh;
  ItemIndex: Integer; InItemPos: TPoint; Shift: TShiftState);
begin
end;

procedure TPopupListboxItemEh.MouseUp(Sender: TPopupListboxEh;
  ItemIndex: Integer; InItemPos: TPoint; Button: TMouseButton;
...
function TPopupListboxItemEh.CanFocus(Sender: TPopupListboxEh;
  ItemIndex: Integer): Boolean;
begin
  Result := True;
end;

{function TPopupListboxItemEh.IsMouseUpCloseListbox(Sender: TPopupListboxEh; ItemIndex: Integer;
  InItemPos: TPoint; Button: TMouseButton; Shift: TShiftState): Boolean;
begin
  Result := CloseOnExecute(Sender, ItemIndex);
end;
}

function TPopupListboxItemEh.CloseOnExecute(Sender: TPopupListboxEh; ItemIndex: Integer): Boolean;
begin
  Result := True;
end;

procedure TPopupListboxItemEh.Execute(Sender: TPopupListboxEh;
  ItemIndex: Integer; InItemPos: TPoint; Shift: TShiftState);
begin
end;

procedure TPopupListboxItemEh.KeyPress(Sender: TPopupListboxEh;
...
//  ShowMessage(GraphicProviderClass.ClassName + ' was registered.')
{$ENDIF}
end;
...
{
procedure TEditButtonImagesEh.SetControlHotImages(const Value: TCustomImageList);
begin
  if FControlHotImages <> Value then
  begin
    FControlHotImages := Value;
    FOwner.Changed;
  end;
end;

procedure TEditButtonImagesEh.SetControlHotIndex(const Value: Integer);
begin
  if FControlHotIndex <> Value then
  begin
    FControlHotIndex := Value;
    FOwner.Changed;
  end;
end;
}
...
begin
  ...
  case Msg.Msg of
    ...
    WM_KEYDOWN:
      ...
      begin
//        if FSelectedItemID <> MaxInt then
